...
function cardHTML(c: Candidate, leaving: boolean): string {
  ...
  const rows = CANDIDATE_CRITERIA.map(
    (label, k) =>
      `<dt>${label}</dt><dd data-v="${c.criteria[k]}">${EVIDENCE_LABEL[c.criteria[k]]}</dd>`,
  ).join("");
  const id = pad(c.id);
  ...
  return (
    ...
    `aria-label="Candidate ${id}, ${c.role}, ${c.industry}. Activate for evaluation detail.">` +
    `<span class="cand-top"><span class="cand-id">Candidate ${id}</span>` +
    `<span class="cand-yrs">${c.years} yrs</span></span>` +
    `<span class="cand-role">${c.role}</span>` +
    `<span class="cand-meta">${c.industry} · ${c.teamSize} team · ${c.scope}</span>` +
    `<span class="crit" aria-hidden="true">${crit}</span>` +
    `<span class="cand-detail"><dl>${rows}</dl>` +
    `<span class="sw"><b>Strength:</b> ${c.strength}</span>` +
    `<span class="sw"><b>Watch point:</b> ${c.watchPoint}</span></span>` +
    `<span class="cand-why">${c.exitReason}</span>` +
    `</div>`
  );
}
...
const finalistHTML = (c: Candidate): string =>
  ...
  `<span class="cand-yrs">${c.years} yrs</span></span>` +
  `<h4 class="cand-role">${c.role}</h4>` +
  `<span class="cand-meta">${c.industry} · ${c.teamSize} team · ${c.scope}</span>` +
  `<p class="sw"><b>Why they are here:</b> ${c.strength}</p>` +
  `<p class="sw"><b>Still to explore:</b> ${c.watchPoint}</p>` +
  `</article>`;
...
export function initShortlist(): void {
  ...
  req.innerHTML = CANDIDATE_CRITERIA.map(
    (r) => `<li><span class="k" aria-hidden="true"></span>${r}</li>`,
  ).join("");

  funnel.innerHTML = FUNNEL.map(
    (f, k) =>
      (k ? '<span class="fn-arrow" aria-hidden="true">→</span>' : "") +
      `<span class="fn"><b>${f.count}</b><span>${f.label}</span></span>`,
  ).join("");
