	...
	<div class="wrap">
		<div class="geo-grid">
			...
			<div class="rv" style="--d:1">
				...
				<div class="geo-links"><?php foreach ( hsg_lines( 'geo_counties' ) as $c ) { echo '<a class="is-county" href="' . esc_url( hsg_city_hub_url() ) . '">' . esc_html( $c ) . '</a>'; } foreach ( hsg_lines( 'geo_towns' ) as $c ) { $u = hsg_city_url_by_name( $c ); echo '<a href="' . ( $u ? esc_url( $u ) : hsg_anchor( 'close' ) ) . '">' . esc_html( $c ) . '</a>'; } ?></div>
			</div>
		</div>
	</div>
</section>
